...
const schema = z.object({
  ...
  category: z.enum(['bolo', 'torta', 'salgado', 'bebida', 'doce', 'acai', 'outro']),
...
const categoryLabels: Record<string, string> = {
  ...
  bebida: 'Bebida', doce: 'Doce', acai: 'Açaí', outro: 'Outro',
};
...
export default function RecipeForm({ recipe, onClose }: Props) {
  ...
  const compInitial = (recipe as any)?.complementos?.length ? (recipe as any).complementos.join('\n') : '';
  const [complementosText, setComplementosText] = useState(compInitial);
  const fileInputRef = useRef<HTMLInputElement>(null);
  useEffect(() => {
    setComplementosText((recipe as any)?.complementos?.length ? (recipe as any).complementos.join('\n') : '');
  }, [recipe?.id]);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      const complementos = data.category === 'acai' && complementosText.trim()
        ? complementosText.split(/\n/).map(s => s.trim()).filter(Boolean)
        : (data.category === 'acai' ? [] : null);
      const payload: any = {
        name: data.name,
        description: data.description || null,
        category: data.category,
        complementos: data.category === 'acai' ? (complementos || []) : null,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {category === 'acai' && (
        <div className="space-y-2 rounded-lg border border-border/50 p-4">
          <Label className="text-sm font-medium">Complementos disponíveis</Label>
          <p className="text-xs text-muted-foreground">Um complemento por linha. Ex.: Morango, Banana, Leite condensado, Leite ninho, Granola.</p>
          <Textarea
            placeholder="Morango&#10;Banana&#10;Leite condensado&#10;Leite ninho&#10;Granola"
            value={complementosText}
            onChange={e => setComplementosText(e.target.value)}
            className="resize-none min-h-[100px] font-sans"
            maxLength={800}
          />
        </div>
      )}
