...
const categoryGradients: Record<string, string> = {
  ...
  acai: 'linear-gradient(135deg, hsl(270 50% 40% / 0.12), hsl(270 50% 40% / 0.04))',
...
export default function RecipeCard({ recipe, index = 0, readOnly = false }: { recipe: Recipe; index?: number; readOnly?: boolean }) {
  ...
  // Por padrão, só considerar que vende "Fatia" automaticamente para bolos/tortas.
  // Para salgados, bebidas e outros, a fatia só aparece se o proprietário marcar explicitamente.
  const defaultSellsSlice =
    recipe.category === 'bolo' || recipe.category === 'torta';
  const sellsSlice = typeof r.sells_slice === 'boolean' ? r.sells_slice : defaultSellsSlice;
