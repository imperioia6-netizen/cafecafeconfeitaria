...
const categoryLabels: Record<string, string> = {
  bolo: 'Bolo', torta: 'Torta', salgado: 'Salgado', bebida: 'Bebida', doce: 'Doce', acai: 'Açaí', outro: 'Outro',
};

// Preço padrão da fatia de bolo no cardápio digital
...
const Cardapio = () => {
  ...
  const dialogPrice = selectedProduct
    ? (selectedProduct.category === 'bolo' && dialogUnitType === 'slice'
        ? CAKE_SLICE_PRICE
                        ...
                        >
                          ...
                          <span className="block text-xs font-mono mt-0.5">
                            R$ {(selectedProduct.category === 'bolo'
                              ? CAKE_SLICE_PRICE
                              : Number(selectedProduct.slice_price || 0)
                            ).toFixed(2).replace('.', ',')}
                          </span>
                      </button>
                      <button
