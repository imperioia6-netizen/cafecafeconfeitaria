...
const Recipes = () => {
  ...
  return (
    <AppLayout>
      <div className="space-y-4 md:space-y-6 min-w-0 max-w-full overflow-hidden">
        ...
        {isOwner && recipes && recipes.length > 0 && (
          <>
            <Tabs value={tab} onValueChange={v => setTab(v as MarginTab)} className="min-w-0 w-full">
              <TabsList className="w-full min-w-0 h-auto gap-1 bg-muted/50 p-1.5 flex flex-wrap">
                {tabConfig.map(t => (
            ...
            <div className="grid grid-cols-fit-3 gap-2 md:gap-3 min-w-0 w-full">
              {[
